...
@task
async def viz_line_masks(params: WorkspaceParams):
    logger = get_logger()
    ...
    def _run() -> dict:
        logger.info("[viz-line] Starting line visualization masks for uuid=%s", upload_uuid)
        ...
        if not geojson_files:
            logger.info("[viz-line] No line GeoJSON files found under %s", input_geojson_dir)
            update_input_progress(upload_uuid, 92)
            notify_api_progress(
                upload_uuid,
                task="4_viz_line.viz_line_masks",
                status_percent=92,
            )
            trigger_result = tirrger_flow("4_viz_poly", upload_uuid)
            return {
                "uuid": upload_uuid,
                "mask_count": 0,
                "next": "4_viz_poly",
        ...
        logger.info("[viz-line] Generated %s line masks in %s", mask_count, output_viz_dir)
        trigger_result = tirrger_flow("4_viz_poly", upload_uuid)
        return {
            "uuid": upload_uuid,
    ...
    try:
        ...
        run_gc_cleanup("viz-line", upload_uuid)


register_pipeline(
    id="4_viz_line",
    description="Generate line layer PNG masks in /tmp/<uuid>/viz before export.",
    tasks=[viz_line_masks],
